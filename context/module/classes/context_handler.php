...
class context_handler extends \local_metadata\context\context_handler {
    ...
    public function require_access() {
        ...
        // UNISTRA DEV.
        require_capability('local/metadata:editmodule', $this->context);
        // End UNISTRA DEV.
    ...
    public function add_settings_to_context_menu($navmenu) {
        ...
        $navmenu->add('modsettings',
                new \admin_externalpage('metadatacontext_modules', get_string('metadatatitle', 'metadatacontext_module'),
                        new \moodle_url('/local/metadata/index.php', ['contextlevel' => $this->contextlevel]),
                        ['moodle/site:config']),
                'managemodulescommon');
        return true;
    }
    ...
    public function extend_settings_navigation($settingsnav, $context) {
        ...
        if ($context->contextlevel == $this->contextlevel) {
            ...
            if ($PAGE->course && ($PAGE->course->id != 1) &&
                ...
                /*UNISTRA DEV*/ && has_capability('local/metadata:editmodule', $context) /*End UNISTRA DEV*/) {
    ...
    public function coursemodule_standard_elements($formwrapper, $mform) {
        global $DB;
        // UNISTRA DEV.
        if (!empty(get_config('local_metadata','coursemodulehookenabled'))) {
        // End UNISTRA DEV.
    ...
    public function coursemodule_edit_post_actions($data, $course) {
        ...
        if ($fields = $DB->get_records('local_metadata_field', ['contextlevel' => $this->contextlevel])) {
            foreach ($fields as $field) {
                ...
                // UNISTRA DEV.
                //check that field exists in data
                if(property_exists($data, 'local_metadata_field_' . $field->shortname)) {
                // End UNISTRA DEV.
                $datachunk->{'local_metadata_field_' . $field->shortname} =
                        $data->{'local_metadata_field_' . $field->shortname};
