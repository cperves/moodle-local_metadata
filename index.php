...
if ($action == $contextname.'data') {
    ...
    $contextrecord = $DB->get_record('context', array('contextlevel'=> $contextlevel, 'instanceid'=> $id));
    require_capability('local/metadata:edit'.$contextname, context::instance_by_id($contextrecord ? $contextrecord->id : context_system::instance()->id));
    // End Unistra DEV.
} else {
